...
def main():
    ...
    while True:
        ...
        screen.fill((0, 0, 0))
        fighter.draw()
        enemy_fleet.draw()  # Movements stay after the game over loop
        scoreboard.draw()

        if is_game_over:
            screen.blit(game_over_image, (170, 200))
            pygame.display.update()
            continue  # will loop to beginning of loop and still let player exit the application
        ...
        for badguy in enemy_fleet.badguys:
            for missile in fighter.missiles:
                if badguy.hit_by(missile):
                    # Increment score or explosion sounds later
        ...
        for badguy in enemy_fleet.badguys:
            if badguy.y > screen.get_height() - fighter.image.get_height() - badguy.image.get_height():
                is_game_over = True
                lose_sound.play()
            # showed image in is_game_over loop above
        ...
        # added into the dead badguy loop above
